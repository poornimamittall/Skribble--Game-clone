...
class MessageHandler {
    ...
    handleJoinRoom(socket, { roomId, playerName }) {
        ...
        if (room.players.size >= (room.settings.maxPlayers || 10)) {
    ...
    handleGuess(socket, { roomId, playerId, text }) {
        ...
        if (correct) {
            ...
            const points = Math.max(
                50,
                Math.round((room.game.timeLeft / room.game.drawTime) * 100) + (10 - guessersCount) * 5
            );
    ...
    handleDrawUndo(socket, { roomId, playerId }) {
        ...
        for (let i = room.canvasState.length - 1; i >= 0; i--) {
            if (room.canvasState[i].type === "end") {
                lastEnd = i;
                break;
            }
        }
