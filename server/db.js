...
function createGameSession(sessionId, roomId, settings) {
    db.prepare(
        `INSERT OR IGNORE INTO game_sessions (id, room_id, rounds, word_count) VALUES (?, ?, ?, ?)`
    ).run(sessionId, roomId, settings.rounds || 3, settings.wordCount || 3);
}
...
function saveGameResult(sessionId, players) {
    ...
    const insertScore = db.prepare(
        `INSERT INTO game_scores (session_id, player_id, player_name, score, is_winner) VALUES (?, ?, ?, ?, ?)`
    );

    const upsertPlayer = db.prepare(
        `INSERT INTO players (id, name, total_score, games_played, wins)
         VALUES (?, ?, ?, 1, ?)
...
function getLeaderboard(limit = 10) {
    return db.prepare(
        `SELECT name, total_score, games_played, wins FROM players ORDER BY total_score DESC LIMIT ?`
    ).all(limit);
}

function getPlayerStats(playerId) {
    return db.prepare(
        `SELECT id, name, total_score, games_played, wins, created_at FROM players WHERE id = ?`
    ).get(playerId);
}

function getRecentGames(limit = 5) {
    return db.prepare(
        `SELECT gs.id, gs.room_id, gs.rounds, gs.started_at, gs.ended_at,
                sc.player_name, sc.score, sc.is_winner
         FROM game_sessions gs
         JOIN game_scores sc ON sc.session_id = gs.id
